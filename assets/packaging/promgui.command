...
class PromHelpers(object):
    ...
    def check_update(self):
        script = '''tell application "Terminal"\nactivate\ndo script "%s/get-latest-prom.sh"\nend tell''' % os.getcwd()
        result, error = NSAppleScript.alloc().initWithSource_(script).executeAndReturnError_(None)
        return result, error
    ...
    def import_rootca(self, certfile):
        ...
        cmds.append('security delete-certificate -c Prom')
...
class PromMacOS(NSObject):

    console_color = ColorSet[0]

    def applicationDidFinishLaunching_(self, notification):
        self.helper = PromHelpers()
        self.setupUI()
        self.startProm()
        self.notify()
    ...
    def setupUI(self):
        ...
        # Set initial image
        raw_data = base64.b64decode(''.join(PROM_ICON_DATA.strip().splitlines()))
        ...
        # Set a tooltip
        self.statusitem.setToolTip_(PROM_TITLE)
        ...
        self.console_window.setTitle_(PROM_TITLE)
    ...
    def startProm(self):
        cmd = '%s/prom' % os.path.dirname(os.path.abspath(__file__))
    ...
    def notify(self):
        ...
        notification.setTitle_("Prom macOS Started.")
...
def precheck():
    ...
    if not has_user_json:
        ...
        alert.setMessageText_('Prom Information:')
        alert.setInformativeText_('Please configure your prom at first.\nFor example, add a new gae.user.json')
