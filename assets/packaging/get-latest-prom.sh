...
case $(uname -s)/$(uname -m) in
	Linux/x86_64 )
		FILENAME_PREFIX=prom_linux_amd64
		;;
	Linux/i686|Linux/i386 )
		FILENAME_PREFIX=prom_linux_386
		;;
	Linux/aarch64|Linux/arm64 )
		FILENAME_PREFIX=prom_linux_arm64
		;;
	Linux/arm* )
		FILENAME_PREFIX=prom_linux_arm
		if grep -q ld-linux-armhf.so ./prom; then
			FILENAME_PREFIX=prom_linux_arm_cgo
		fi
		;;
	Linux/mips64el )
		FILENAME_PREFIX=prom_linux_mips64le
		;;
	Linux/mips64 )
		FILENAME_PREFIX=prom_linux_mips64
		;;
	Linux/mipsel )
		FILENAME_PREFIX=prom_linux_mipsle
		;;
	Linux/mips )
		FILENAME_PREFIX=prom_linux_mips
		if hexdump -s 5 -n 1 $SHELL | grep -q 0001; then
			FILENAME_PREFIX=prom_linux_mipsle
		fi
		;;
	FreeBSD/x86_64 )
		FILENAME_PREFIX=prom_freebsd_amd64
		;;
	FreeBSD/i686|FreeBSD/i386 )
		FILENAME_PREFIX=prom_freebsd_386
		;;
	Darwin/x86_64 )
		FILENAME_PREFIX=prom_macos_amd64
		;;
	Darwin/i686|Darwin/i386 )
		FILENAME_PREFIX=prom_macos_386
		;;
	* )
...
if ./prom -version >/dev/null 2>&1; then
	PROM_OS=$(./prom -os)
	PROM_ARCH=$(./prom -arch)
	if test "${PROM_OS}" = "darwin"; then
		PROM_OS=macos
	fi
	FILENAME_PREFIX=prom_${PROM_OS}_${PROM_ARCH}
fi

LOCALVERSION=$(./prom -version 2>/dev/null || :)
...
echo "1. Checking Prom Version"
if test "${LATEST}" = "false"; then
	FILENAME=$(curl -k https://github.com/phuslu/promci/commits/master | grep -oE "${FILENAME_PREFIX}-r[0-9]+.[0-9a-z\.]+" | head -1)
else
	FILENAME=$(curl -kL https://github.com/phuslu/promci/releases/latest | grep -oE "${FILENAME_PREFIX}-r[0-9]+.[0-9a-z\.]+" | head -1)
fi
...
curl -kL https://github.com/phuslu/promci/releases/download/${REMOTEVERSION}/${FILENAME} >${FILENAME}.tmp
