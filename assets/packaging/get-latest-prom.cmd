...
reg query "HKLM\Hardware\Description\System\CentralProcessor\0" | find /i "x86" && (
    set filename_prefix=prom_windows_386
) || (
    set filename_prefix=prom_windows_amd64
)

if exist "prom.exe" (
    for /f "usebackq" %%I in (`prom.exe -version`) do (
...
if not "%localversion%" == "" (
    echo 0. Local Prom version %localversion%
)

set remoteversion=
(
    title 1. Checking Prom Version
    echo 1. Checking Prom Version
    cscript /nologo ~gdownload.vbs https://github.com/phuslu/promci/commits/master ~prom_tag.txt
) && (
    for /f "usebackq tokens=2 delims=-." %%I in (`findstr "%filename_prefix%-r" ~prom_tag.txt`) do (
        set remoteversion=%%I
    )
) || (
...
(
    ...
    cscript /nologo ~gdownload.vbs https://github.com/phuslu/promci/releases/download/!remoteversion!/%filename% "~%filename%"
    if not exist "~%filename%" (
        echo Cannot download %filename%
        goto quit
    )
) && (
    title 3. Extract Prom files
    echo 3. Extract Prom files
    move /y ~%filename% ~%filename%.exe
    del /f ~gdownload.vbs 2>NUL
    for %%I in ("prom.exe" "promgui.exe") do (
        if exist "%%~I" (
            move /y "%%~I" "~%%~nI.%localversion%.%%~xI.tmp"
        )
    )
    ~%filename%.exe -y || echo "Failed to update Prom, please retry."
)
