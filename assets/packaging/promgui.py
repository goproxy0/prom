...
if platform.mac_ver()[0] > '10.':
    sys.exit(os.system(
        'osascript -e \'display dialog "Please use Prom.app rather than promgui.py" buttons {"OK"} default button 1 with icon caution with title "Prom GTK"\''))
...
try:
    ...
    sys.exit(os.system(
        'gdialog --title "Prom GTK" --msgbox "Please install python-gtk2" 15 60'))
try:
    import pynotify
    pynotify.init('Prom Notify')
except ImportError:
    pynotify = None
try:
    import appindicator
...
def rewrite_desktop(filename):
    ...
    with open(filename, 'wb') as fp:
        content = re.sub(r'(?m)Exec=.*', '''Exec=bash -c "export PATH=$PATH:`dirname '%%k'`:%s; exec promgui.py"''' % os.getcwd(), content)
        content = re.sub(r'Icon=\S*', 'Icon=%s/promgui.png' % os.getcwd(), content)
        fp.write(content)
...
class PromGTK:
    ...
    command = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'prom'), '-v=2']
    message = u'Prom already started.'
    fail_message = u'Prom start failed, see the terminal'
    ...
    def __init__(self, window, terminal):
        ...
        if appindicator:
            self.trayicon = appindicator.Indicator(
                'Prom', 'indicator-messages', appindicator.CATEGORY_APPLICATION_STATUS)
    ...
    def show_notify(self, message=None, timeout=None):
        if pynotify and message:
            notification = pynotify.Notification('Prom Notify', message)
